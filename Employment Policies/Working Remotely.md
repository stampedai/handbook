...
If and when the current company setup changes, we should address the above situations. For the purpose of this document, we interpret remote work as as a co-located employee of Stamped working from home or from somewhere irregular.
...
The majority of policies that support remote work in Stamped apply to everyone at Stamped, not just remote workers. The primary documents that outline these policies are:
...
Working remotely can add a financial burden on an individual. At the same time, it is also a privilege. While Stamped is on its seed extension, Stamped will subsidize co-working space costs for up to $[TODO]/day. Once Stamped reaches profitability or raises its next major fundraising round, we will revisit this subsidy.
...
This is a time where employees can voice concerns about constraints that are limiting their ability to work effectively: this could be concerns about structures inside of Stamped that limit their flexibility, concerns about how the way other teammates work, or anything else that might affect how they work. It's also a time where managers should give concrete feedback on whether an employee is upholding their responsibilities when they are working remotely.
